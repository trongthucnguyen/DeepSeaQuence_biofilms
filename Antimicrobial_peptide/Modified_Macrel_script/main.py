...
def parse_args(args):
    ...
    parser.add_argument('--longreads', required=False,
            help='Long-read FASTQ file for mapping to reference AMPs', dest='longreads')
...
def validate_args(args):
    ...
    elif args.command == 'longreads-abundance':
        if not args.fasta_file:
            error_exit(args, "Fasta reference file is required for 'longreads-abundance'")
        if not args.longreads:
            error_exit(args, "Long-read FASTQ file is required for 'longreads-abundance'")
    else:
...
def do_longreads_abundance(args, tdir, logfile):
    import pandas as pd

    # Uncompress/reference FASTA
    ref_fasta = link_or_uncompress_fasta_file(args.fasta_file, path.join(tdir, 'ref.faa'))

    # Run minimap2
    sam_file = path.join(tdir, 'minimap2.out.sam')
    subprocess.check_call([
        'minimap2', '-a', '-x', 'map-ont', ref_fasta, args.longreads
    ], stdout=open(sam_file, 'w'), stderr=logfile)

    # Convert to BAM, sort, and index
    bam_file = path.join(tdir, 'minimap2.out.sorted.bam')
    subprocess.check_call(['samtools', 'view', '-bS', sam_file], stdout=open(bam_file+'.unsorted', 'wb'))
    subprocess.check_call(['samtools', 'sort', bam_file+'.unsorted', '-o', bam_file])
    subprocess.check_call(['samtools', 'index', bam_file])

    # Count reads per reference
    count_file = path.join(args.output, args.outtag + '.counts.txt')
    with open(count_file, 'w') as out:
        subprocess.check_call(['samtools', 'idxstats', bam_file], stdout=out)

    # Convert to TPM
    # Format: seq_name \t length \t mapped_reads \t unmapped_reads
    df = pd.read_csv(count_file, sep='\t', header=None,
                     names=['AMP_ID', 'Length', 'Mapped', 'Unmapped'])
    df = df[df['Length'] > 0]  # avoid division by zero
    df['RPK'] = df['Mapped'] / (df['Length'] / 1000)
    total_rpk = df['RPK'].sum()
    df['TPM'] = 1e6 * df['RPK'] / total_rpk
    df[['AMP_ID', 'Length', 'Mapped', 'TPM']].to_csv(
        path.join(args.output, args.outtag + '.tpm.tsv'),
        sep='\t', index=False)
...
def main(args=None):
    ...
    with tempfile.TemporaryDirectory(dir=args.tmpdir) as tdir:
        ...
        if args.command == 'longreads-abundance':
            do_longreads_abundance(args, tdir, logfile)
            with open_output(os.path.join(args.output, 'README.md')) as ofile:
                ofile.write("# Macrel Long-read Abundance Mapping\nMapped Nanopore reads to AMP reference and calculated TPM values.")
