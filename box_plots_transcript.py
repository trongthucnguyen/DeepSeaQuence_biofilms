...
# Load data
df = pd.read_csv("transcript_abundance.csv") 

# Melt the dataframe to long format
tpm_cols = [col for col in df.columns if 'lg(TPM)' in col]
...
df_melted = df_melted[df_melted['TPM'] > 0]
...
for cls in classes_of_interest:
    class_data = df_melted[df_melted['class'] == cls]['TPM']
...
plt.title('Transcript abundance')
plt.ylabel('Transcript abundance (lgTPM)')
plt.xlabel('BGC Class')
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig("transcript_boxplot.svg", format="svg")
plt.show()
